...
| E4 | Distributed worker scaling is NOT linear: 2 workers ≈ 1.08–1.14x, 4 workers slower than 1 (0.81–0.87x) for fine-grained synthetic tasks | same as E3 | same `scaling/` artifact | `b4b651d` | GREEN (as a measured limitation) | Preserved, not hidden: TS workers parallelize internally via async, so extra processes add claim/contention overhead; bottleneck is the single-threaded result-consumption loop + Redis round-trips. Do NOT claim "linear scaling" for the distributed runtime. **RESOLVED in M41** (commit `ee304d3`+): batched result consumption removed the regression and lifted absolute throughput — see E16; flat-vs-worker-count behavior remains for this workload shape by design (see E16 note). |
...
| E16 | Batched result consumption (M41: XREADGROUP COUNT n + one multi-id XACK per batch, apply-then-ack with the durable idempotency ledger as the crash backstop) raised sustained 500-task pipeline throughput from 107.8 → 182.3 tasks/s median at 1 worker (+69%) and eliminated the multi-worker throughput regression: 4-worker relative scaling improved 0.81x → 0.998x | `ctest --test-dir engine/build -R m39_scaling` | `.../20260823-163858_m41_ee304d3/scaling/{manifest.json,samples.jsonl,summary.json}` (2 trials/cell; durable-store audit per trial) | `ee304d3`+ | GREEN | Scaling vs worker count remains ~flat BY DESIGN for this workload shape: one async TS worker already overlaps the 20 ms synthetic sleeps, so extra processes add no execution capacity once the scheduler pipeline is no longer the cap. Flat ≠ regression. Per-cell loop timings (batches/results/dispatch_ms/consume_ms) are printed per trial and batch size recorded in manifest.json. |
| E17 | Distributed runtime completes 1,000-task wide DAGs end-to-end across 1/2/4 TS workers at 175–214 tasks/s median throughput with zero lost or duplicated tasks (durable-store audit: every task succeeded, exactly one attempt per task) | `EVO_M39_TASKS=1000 ctest -R m39_scaling` | `.../20260823-163858_m41_ee304d3/scaling-1000/{manifest.json,samples.jsonl,summary.json}` | `ee304d3`+ | GREEN | Synthetic I/O-bound tasks on a single local host; extends the E3 audit range (100–500) to 1,000. |
| E18 | Weighted fair scheduling measured under explicit 2:1 entitlements (org weights {2,1}, unequal demand 2T vs T): contended-window service ratio 1.75 vs 1.00 equal-weight control; Jain(normalized contended service) 0.996; final served counts exactly 2:1; no starvation | `ctest --test-dir engine/build -R fairness_bench` (workload C + control) | `.../20260823-163858_m41_ee304d3/faults/fairness/summary.json` (`workload_weighted_*`) | `ee304d3`+ | GREEN | Slot-grant-level fairness (browser affinity still serializes a tenant's backlog); grant order is a deterministic function of gate state. First measured weighted-fairness evidence (E9 covered equal weights only). |
| E19 | SIGKILL worker-recovery latency distribution over 7 trials: median 6,468 ms (min 6,347 / max 6,508); every trial recovered with no lost task and reassignment to a surviving worker | `EVO_M34_TRIALS=7 ctest -R crash_recovery` | `.../20260823-163858_m41_ee304d3/faults/crash_recovery/summary.json` | `ee304d3`+ | GREEN | Recovery bounded by lease duration + scan interval in the test config; single local stack. |
