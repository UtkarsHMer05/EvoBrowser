...
| # | Decision | Rationale | Consequences |
|---|----------|-----------|--------------|
| 32.1 | Error taxonomy is a fixed enum: transient, resource_lost, permanent, validation, authorization, canceled, unknown — classified from the proto `ErrorClass` the worker reports. | M32 step 1: retryability must be attributable to a named category, not ad-hoc string matching. The proto enum is the wire contract; the C++ `ErrorCategory` is the policy input. | `classify_error()` is the single mapping point; new wire classes default to `Unknown` (forward compatible). |
| 32.2 | Default retry policy is per RESOURCE CLASS (`RetryPolicySet::for_class`): internal = 3 attempts / 100ms base; browser = 2 attempts / 500ms base (sessions are expensive); external_io = 1 attempt (NO retry by default). | M32 step 2 + no-go: side-effecting external calls (email) must not be retried without an idempotency strategy (that is M33's job). Browser retries are conservative because a session re-open is costly. Legacy Trigger.dev behavior is untouched — this is an Evo-only path. | `external_io` nodes fail fast on first failure unless a later milestone adds an idempotency strategy; documented, not a bug. |
| 32.3 | Policy floor: permanent / validation / authorization / canceled are NEVER retried, even if the worker set `retryable=true`. Unknown is retried ONLY on an explicit positive worker hint; a worker hint of `retryable=false` overrides a default-retryable class. | M32 no-go: do not retry authorization/validation blindly. The scheduler's taxonomy outranks the worker's hint for never-retry classes; the hint only ADDS retryability for unknown, or REMOVES it explicitly. | A misbehaving worker cannot force a retry of an auth failure (asserted in retry_policy_test). |
| 32.4 | Attempt budget: `failed_attempt >= max_attempts` ⇒ dead-letter. The attempt number comes from the result envelope, so lease-expiry re-dispatch (M31) consumes NO retry budget — a reaped attempt is recovery, not a retry. | M31 step 5 + M32 step 9: a lost worker must not burn the node's retry budget. The two mechanisms compose: reap → re-dispatch as a new attempt → if THAT fails transiently, the retry policy applies normally. | Asserted in run-loop test 18: killed attempt reaped, then transient fail, then success on attempt 3 (3 attempts total, budget intact). |
| 32.5 | Backoff = `base * multiplier^(attempt-1)` capped at `max_backoff`, scaled by a deterministic jitter factor in `[1-j, 1+j]` from a per-(node, attempt, run-seed) FNV-1a-seeded xorshift64*. | M32 step 4: exponential backoff with BOUNDED jitter and a DETERMINISTIC seed so retry timing is reproducible in tests. Jitter decorrelates concurrent retries without making tests flaky. | Same (node, attempt, seed) ⇒ same backoff; `retry_jitter_seed` is config (default `0xC0FFEE`). Backoff-bounds asserted in retry_policy_test. |
| 32.6 | Backoff is realized as STATE + due-time, never a blocked thread: the node parks in `RETRY_WAIT` with a persisted `retry_wait_until` (wall-clock UTC ms) + `retry_reason`; `process_retry_waits()` re-readies due nodes each loop iteration. | M32 step 6: the run loop is single-threaded; sleeping would stall every other run/node. A due-time map + per-iteration check is the timer equivalent. | No thread blocked during backoff; `retry_due_` is loop-owned state (no new synchronization). |
| 32.7 | Durable retry evidence: `node_runs.retry_wait_until` + `node_runs.retry_reason` (migration 0005, additive, LOCAL PG only); `set_node_retry_wait()` is guarded against terminal nodes in both stores. | M32 step 5: the next-attempt time and reason must survive a scheduler restart and be auditable. Additive migration keeps Phase-1 tables untouched. | Neon untouched (needs explicit human approval); the reconnect snapshot maps `retry_wait` → running so a parked node never reads as failed mid-backoff. |
| 32.8 | Exhaustion is terminal: `dead_letter_node()` moves the node to `DEAD_LETTERED`, cancels downstream, and `finalize_run()` counts `DeadLettered` as failure (run finalizes FAILED, not CANCELED). | M32 step 7: exhausted retries are a logical failure, not a cancellation. Counting it in `any_failed` was a fix found this session — without it a dead-lettered run finalized as CANCELED. | `node_dead_lettered` event + `NODE_STATE_DEAD_LETTER` gRPC state + UI `failed` mapping; asserted in run-loop test 16. |
| 32.9 | Retry/dead-letter transitions are guarded by the state machine (`retry_wait_node` requires RUNNING; `ready_from_retry` requires RETRY_WAIT; `dead_letter_node` is terminal) and by the store's terminal guard, so cancellation racing backoff wins cleanly. | M32 step 9 (cancellation during backoff) + concurrency question "what happens when cancellation races completion": a parked node must be canceled, never re-dispatched, after a cancel request. `process_retry_waits()` also short-circuits once `cancel_requested_` is set. | Asserted in run-loop test 17: node parked on a long backoff is canceled (not re-dispatched) and the run finalizes CANCELED. |
| 32.10 | UI/diagnostics exposure is deliberately minimal: `node_retry_scheduled` keeps the step reading as RUNNING (transient failures are not surfaced as terminal), `node_dead_lettered` reads as FAILED with the reason; durable `retry_wait` maps to running in the snapshot. Attempt history stays in `task_attempts` + events for diagnostics, not the normal UI. | M32 step 8: expose retry history to diagnostics WITHOUT overwhelming the normal UI. A node in backoff is still in progress from the user's perspective. | Three mapping surfaces updated consistently: `reduceEvoEvents` (event fold), `mapNodeStatus` (durable snapshot), gRPC `GetRun` (engine route). |
| 32.11 | Timestamps: `retry_wait_until` and the due-time comparison are wall-clock UTC ms (durable boundary); steady_clock is never persisted for retry. | M32 step 17: consistent wall-clock at every durable boundary, matching M31's lease timestamps. | Retry due-times are clock-consistent across the C++ scheduler, PG, and any future TS consumer. |
| 32.12 | No performance numbers claimed; retry timing in tests is diagnostic, not benchmark evidence. | M32 performance-evidence rule: no resume number without benchmark methodology. | Retry-latency/throughput benchmarking deferred to M39. |

## M33 — Implement idempotency and duplicate suppression

| # | Decision | Rationale | Consequences |
|---|----------|-----------|--------------|
| 33.1 | The logical operation key for result application is derived from the ATTEMPT identity: `result:{run_id}:{node_id}:{attempt_number}` (`result_idempotency_key`). | M33 step 1: the key must be deterministic and tied to the operation semantics (applying THIS attempt's result), so a duplicate delivery of the same result claims the same key. Transport message ids are NOT identity (M22 rule). | Same envelope => same key; a different attempt (retry/re-dispatch) is a DIFFERENT key and is not suppressed by this gate. |
| 33.2 | Durable idempotency ledger = the existing `idempotency_records` table (M19), whose `key` is the PRIMARY KEY. `claim_idempotency_key` uses `INSERT ... ON CONFLICT (key) DO NOTHING`; first claim wins, a duplicate affects 0 rows. | M33 step 2: a durable record with a UNIQUE constraint is the authoritative duplicate gate. Reusing the M19 table (already migrated) means NO new migration and no schema churn. | Implemented in both `InMemoryRunStore` (map key = unique constraint) and `PgRunStore` (parameterized SQL). The committed response is stored and readable for reuse. |
| 33.3 | The durable ledger is wired into `apply_result` as the authoritative duplicate-result gate, placed AFTER the in-memory `ResultDedupe` fast path and BEFORE the late-result rule. | M33 step 3: the in-memory dedupe is lost on a scheduler restart; the durable ledger survives it. Keeping the in-memory set as the fast path avoids a durable round-trip for same-process redelivery. | A duplicate result (same attempt) is suppressed durably: it never re-applies, never double-unlocks successors, never double-frees a resource slot. The caller acks both applied and suppressed results. |
| 33.4 | For a duplicate SUCCESS delivery, the ledger stores the first committed output (`result.output()`); `get_idempotency_response` returns it so a duplicate can reuse the previously committed output. | M33 step 4: pure/read result application reuses the committed output rather than re-deriving it. The ledger is the source of truth for "what was committed". | Asserted in the dup-storm test: the ledger response equals the node's persisted output exactly (whichever result won the race). |
| 33.5 | Side-effecting executors get a deterministic idempotency key derived from the LOGICAL OPERATION (run + node), NOT the attempt number: `side-effect:{run_id}:{node_id}`. | M33 steps 5–6 + no-go: this is what closes the crash window. If a worker performs the side effect and dies before the result is durably applied, the scheduler reaps the lease (M31) and re-dispatches as a NEW attempt — but the re-execution reuses the SAME key, so the provider returns the original effect instead of performing it twice. A whole-workflow re-run is a NEW run id => a new key (intended fresh send). | A legitimate retry/re-dispatch does NOT re-send email; a re-run does. The key is stable across attempts by design. |
| 33.6 | `send-email` forwards the key to Resend as the `Idempotency-Key` header (SDK 6.17.2 `idempotencyKey` option, confirmed supported in the installed SDK + Resend docs). Undefined key (legacy path) => Resend's default non-idempotent behavior, unchanged. | M33 step 5: inspect current provider support before choosing the strategy. Resend DOES support provider-side idempotency, so we use it rather than documenting an unavoidable window. | Provider-side dedup is the primary guard for the external side effect. The legacy Trigger.dev path is untouched (no key passed). |
| 33.7 | The unavoidable residual ambiguity is documented, not hidden: provider-side idempotency protects against duplicate DELIVERY of the same logical operation, but if the provider call itself is ambiguous (e.g. a network timeout after the provider committed but before the worker saw the response), the worker cannot know whether the effect happened. Retrying with the same key is safe (provider dedupes), but the FIRST ambiguous call's outcome is not locally knowable. | M33 step 6 + no-go: never claim exactly-once. The ledger + provider key eliminate duplicate-application and duplicate-delivery side effects, but not the single ambiguous in-flight call. | Documented in PROGRESS.md known-limitations and the evidence table. This is inherent to at-least-once + external side effects. |
| 33.8 | The deterministic fake email sink (`createEmailTestSink`) mirrors Resend's provider-side dedup: a repeated `idempotencyKey` returns the cached result WITHOUT recording a new send. Tests count ACTUAL side effects via `sent.length`. | M33 step 7: a deterministic fake sink lets failure-injection tests count real duplicate side effects without sending email. | The M24 parity tests still pass (sink output shape unchanged); the M33 tests assert exactly-one-send under duplicate delivery and crash-recovery re-dispatch. |
| 33.9 | Duplicate classes injected and asserted (M33 step 8): duplicate task delivery (worker executes both, provider dedupes => 1 send), duplicate result delivery (scheduler ledger suppresses => 1 application), crash-after-result / lease-reap re-dispatch (same side-effect key => 1 send), and lost-ack redelivery (covered by the dup-storm + at-least-once transport). | M33 step 8: exercise each duplicate class with evidence. | Each class has a named test: run-loop dup-storm (duplicate result), PG ledger unique-constraint, adapter dup/re-dispatch/re-run, worker duplicate-task-delivery. |
| 33.10 | Evidence table (M33 step 9) — what is suppressed vs. what remains ambiguous: SUPPRESSED: duplicate result application (durable ledger), duplicate successor unlock, duplicate resource-slot free, duplicate task-delivery side effect (provider key), crash-recovery re-dispatch side effect (same key). REMAINING AMBIGUITY: a single ambiguous in-flight provider call (timeout after provider commit) — outcome not locally knowable; safe to retry with the same key. | M33 step 9: an honest evidence table of exactly what duplicate classes are suppressed and which ambiguity remains. | Recorded in PROGRESS.md. Never claim exactly-once (M33 no-go). |
| 33.11 | No new migration: `idempotency_records` already exists from M19 (key PK, run_id, response jsonb, created_at). The C++ store and TS worker share it. | M33 step 2 + repo convention: reuse the existing durable structure rather than creating a parallel one. Additive-only evolution; no Phase-1 table touched. | No migration file added this milestone; the table was applied to local PG in M19. Neon untouched. |
| 33.12 | No performance numbers claimed; duplicate-suppression timing in tests is diagnostic, not benchmark evidence. | M33 performance-evidence rule. | Throughput/latency under duplicate load deferred to M39. |

## M34 — Implement worker crash recovery and failure injection

| # | Decision | Rationale | Consequences |
|---|----------|-----------|--------------|
| 34.1 | Fault injection kills the lease-holding worker's ENTIRE process group (`POSIX_SPAWN_SETPGROUP` + `kill(-pid, SIGKILL)`), never just the spawned pid. | `npx tsx` spawns a 3-level tree (`npm exec` → `node tsx` → `node main.ts`). Killing only the spawned pid leaves the real worker running, which defeats the crash simulation (the "killed" worker finishes its own task) and orphans a process that holds CTest's output pipe open. | The crash test kills the real worker deterministically; worker stdout/stderr go to `/dev/null` so no orphan can wedge CTest on pipe EOF. |
| 34.2 | Recovery is demonstrated on a REAL multi-process fleet (C++ run loop + 2 spawned TS workers over real Redis + Postgres), with the kill targeted at the worker that actually holds the lease on the slow task. | M34 no-go: "a test that only kills an idle worker is insufficient." The kill must hit an ACTIVE, lease-holding worker mid-task for the claim to mean anything. | The test waits for a worker to acquire the `(slow, attempt 1)` lease, records the injection timestamp, then SIGKILLs that exact worker. |
| 34.3 | Lease/heartbeat cadence is env-configurable (`EVO_WORKER_LEASE_DURATION_MS` / `_RENEW_INTERVAL_MS` / `_HEARTBEAT_INTERVAL_MS`), with production defaults unchanged when unset. | The crash test needs SHORT leases (fast reap) to keep trial time bounded; production keeps the generous M31 defaults. Config at the `main.ts` boundary means `Worker`'s defaults are untouched. | Setting these does NOT change production behavior; the test uses 1500ms lease / 400ms renew / 300ms heartbeat. |
| 34.4 | The late-result rule is checked BEFORE the durable idempotency claim in `apply_result`. | A forged/late result (older attempt) must never pollute the idempotency ledger. The ledger is the authoritative duplicate gate; letting a stale result claim a key would corrupt the "first committed output" it stores. | A late completion from a reaped attempt is rejected before it can claim the ledger key; asserted by the existing dup-storm + late-result tests. |
| 34.5 | Browser-affinity worker death follows the documented resource-loss policy: the capacity-1 affinity slot is RELEASED on lease reap, the node re-dispatched as a new attempt on a FRESH session, and the chain continues. No transparent session continuation is claimed. | FAILURE_MODEL.md §4 + master prompt M34 step 6: a crashed worker's browser session is treated as LOST. The slot must not leak (or the run would deadlock on the capacity-1 key), and we must not claim session continuation we haven't proven. | Asserted in run-loop test 19: killed browser worker's attempt reaped, slot freed, node re-dispatched on a healthy worker, downstream browser node runs on the freed slot, run succeeds. |
| 34.6 | Recovery evidence is recorded as raw per-trial samples (injection / lease-expiry / replacement-acquired / run-complete wall-clock ms + derived reap/reassign/recovery latencies) written to `EVO_M34_ARTIFACT_DIR` with commit + hardware + build-mode provenance. | M34 step 7 + BENCHMARK_METHODOLOGY.md: raw samples with provenance, not averages-only prose. Timings are labeled DIAGNOSTIC (single local stack), not evidence-grade benchmark numbers. | Artifacts in `engine/bench-results/m34/` (manifest/samples/summary/command). Evidence-grade recovery benchmarking is M39's job. |
| 34.7 | Recovery behavior is documented separately for synthetic/non-browser work (reassignment on a surviving worker) and browser-affinity work (resource lost + re-created on a fresh session). | M34 step 8 + no-go: "recovery claims must specify workload/resource class." The two classes have different semantics and must not be conflated. | PROGRESS.md records both; the crash test covers synthetic (INTERNAL), the run-loop test covers browser affinity. |
| 34.8 | No performance numbers claimed; recovery latencies are diagnostic samples, not resume metrics. | M34 performance-evidence rule. | Recovery-time benchmarking (multi-trial, evidence-grade) deferred to M39. |
